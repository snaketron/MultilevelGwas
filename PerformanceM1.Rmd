...
m1fast <- rstan::stan_model(file = "src/stan_files/M1fast.stan")
```
...
pfast <- rstan::sampling(object = m1fast,
                     data = d1$data.list,
                     iter = 1000,
                     warmup = 500,
                     chains = 4,
...
```{r, fig.width=5, fig.height=5}
p.s <- summary(p, par = "mu_beta")$summary[, "mean"]
pfast.s <- summary(pfast, par = "mu_beta")$summary[, "mean"]
plot(p.s, pfast.s)
abline(0, 1, col = "red")
```
