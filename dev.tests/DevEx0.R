...
# Remark: 0 strong effects, 30 noise
...
# P <- 1
#
# genotype <- replicate(n = 30, rep(x = sample(x = c("A", "C"),
#                       size = K, replace = T), each = N))
#
#
# # equal means
# mu1.k <- rnorm(n = K, mean = 0, sd = 1)
#
...
#           traits = traits,
#           strains = strains)
# save(d, file = "dev.tests/ex0/d.RData")





d <- get(load(file = "dev.tests/ex0/d.RData"))

mc.ex0 <- runModelComparison(genotype = d$genotype,
                             traits = d$traits,
                             trait.type = c("Q"),
                             strains = d$strains,
                             models = c("M0", "M1"),
...
save(mc.ex0, file = "dev.tests/ex0/mc.ex0.RData")
...
# posteriors
summary(mc.ex0$ps$M0, par = c("nu", "nu_help", "mu_beta"))$summary
summary(mc.ex0$ps$M1, par = c("nu", "nu_help", "grand_mu_beta"))$summary


getTMoments(nu = summary(mc.ex0$ps$M0, par = c("nu"))$summary[1, "mean"],
            mu = summary(mc.ex0$ps$M0, par = c("mu_beta"))$summary[1, "mean"])
getTMoments(nu = summary(mc.ex0$ps$M1, par = c("nu"))$summary[1, "mean"],
            mu = summary(mc.ex0$ps$M1, par = c("grand_mu_beta"))$summary[1, "mean"])


# correlation between snp slopes
plot(summary(mc.ex0$ps$M0, par = c("beta"))$summary[, "mean"],
     summary(mc.ex0$ps$M1, par = c("mu_beta"))$summary[, "mean"])
abline(0, 1)
